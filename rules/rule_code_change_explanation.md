...
- 본문은 개조식 1~4 bullet:
  ...
  - 문법적 특징: 1줄
  - 활용 패턴: 1줄
  - 직관 설명: 변경 내용을 개발 용어 없이 일반어로 충분히 풀어 설명 (ASCII 다이어그램 권장)
- 변경 의미가 사소하면 핵심 동작 1 bullet만 작성 가능
- 직관 설명 작성 원칙:
  - 비유·일상 이미지로 변경 본질 전달, 코드·라이브러리·함수명 등 식별자 노출 회피
  - 한 줄 압축 금지 — 변경 전/후의 달라지는 그림이 손에 잡히도록 자세히 풀어쓰기
  - ASCII 다이어그램으로 흐름·전환·구조를 시각화 (전/후 비교, 입출력 흐름, 분기 구조 등)
  - 길이보다 직관 이해도 우선
...
- 직관 설명: 원래 이 규칙은 한 폴더 입구에만 세워둔 표지판이었다. 그 폴더 안에 들어가는 문서들은 표지판을 따랐지만, 다른 자리에 있는 문서들은 무관했다. 변경 후에는 같은 표지판이 회사 안 모든 문서 책상 위에 한 장씩 복사되어 자동으로 붙는 셈이다. 새 문서를 어디에 만들든 표지판이 따라붙기 때문에, 일일이 손으로 표지판을 세울 필요가 없어진다.

  Before                                After
  ┌────────────┐                        ┌────────────┐
  │  표지판    │ ──적용──▶ [docs 폴더]  │  표지판    │ ──복사──▶ [.md] [.md] [.md] [.md]
  └────────────┘                        └────────────┘            (모든 마크다운 파일)
  (한 폴더 한정)                        (전 디렉토리 자동 적용)
```
```
src/auth.ts:34
...
- 직관 설명: 가게 입구에서 손님이 출입증을 내미는 상황이다. 이 코드는 그 출입증을 받아 검문소로 보낸 뒤, 도장이 진짜인지·유효 기간이 지나지 않았는지·발급처가 신뢰할 만한지 세 가지를 한꺼번에 확인한다. 통과하면 손님 정보를 안쪽으로 넘기고, 한 항목이라도 어긋나면 그 자리에서 돌려보내며 안내문 한 장을 손에 쥐여준다. 통과한 손님은 다음 절차에서 다시 검문받지 않도록 결과를 한 번에 확정해서 넘긴다.
