...
## Step 5. 빌드 실행 테스트
생성된 코드가 실제로 빌드·컴파일되는지 검증한다.

### 5-1. 빌드 환경 판정
- 05_tech_spec.md의 기술 스택을 기반으로 빌드 방식을 결정한다:
  - C/C++: `make` 또는 `cmake`
  - Node.js/TypeScript: `npm install && npm run build` 또는 `tsc`
  - Python: `pip install -r requirements.txt` (의존성 설치 검증)
  - Java/Kotlin: `gradle build` 또는 `mvn package`
  - Go: `go build ./...`
- 로컬 환경에서 빌드가 불가능한 경우 (WSL 미설치, 런타임 미설치 등):
  - Docker 컨테이너 기반 빌드를 시도한다.
  - Docker도 불가능하면 Human에게 보고하고 이 단계를 스킵한다.

### 5-2. 빌드 실행
- 판정된 빌드 명령을 실행한다.
- 빌드 결과를 수집한다:
  - 성공: 경고(warning) 목록 포함하여 보고
  - 실패: 에러 메시지를 분석하여 원인을 파악

### 5-3. 빌드 실패 시 수정
- 에러 원인을 분석하여 수정 방향을 판정한다:
  - 단순 오타/import 오류 → 오케스트레이터가 직접 수정
  - 구조적 문제 (누락된 파일, 잘못된 의존성) → 해당 subagent 재실행
  - 환경 문제 (패키지 미설치 등) → Human에게 보고
- 수정 후 재빌드하여 성공할 때까지 반복한다 (최대 3회).
- 3회 초과 시 Human에게 보고하고 진행 여부를 확인한다.

### 5-4. 스모크 테스트 (선택적)
- 빌드 성공 후, 간단한 실행 테스트를 수행한다:
  - 서버: 프로세스 기동 → 정상 리스닝 확인 → 종료
  - 클라이언트: 서버 연결 → 기본 요청/응답 확인 → 종료
  - CLI 도구: `--help` 또는 기본 명령 실행 확인
- 실행 테스트 중 발견된 버그는 즉시 수정하고 재빌드한다.
- 실행 환경이 없는 경우 (Docker 필요 등) Human에게 보고하고 스킵 가능.

## Step 6. 실행 가이드 생성
빌드된 프로젝트의 실행 방법을 안내하는 GUIDE.md를 생성한다.

### 6-1. 가이드 내용 구성
프로젝트 특성에 맞게 아래 항목을 포함한다:
...
### 6-2. 가이드 생성 규칙
- Step 5 빌드 테스트에서 확인된 실제 명령과 출력을 기반으로 작성한다.
- 추상적 설명이 아닌, 복사-붙여넣기 가능한 구체적 명령을 제공한다.
- 프로젝트 루트($$output_path)에 GUIDE.md로 생성한다.
- 기존 README.md와 중복되지 않도록 한다 (README는 프로젝트 소개, GUIDE는 실행 방법).
...
- 영향 범위 판정 기준:
  - scaffolder 수정 → 전체 재실행 (Step 2~6)
  - infra 수정 → infra만 재실행
  - server/client/db 수정 → 해당 단계만 재실행 + Step 5 재실행
  - 빌드 설정 수정 → Step 5 재실행
  - 실행 가이드 수정 → Step 6만 재실행
...
- 코드 수정이 있었다면 Step 5 (빌드 테스트)를 자동으로 재실행한다.

### 7-4. 반복 판정
- 재실행 결과를 Human에게 제시한다.
- Human이 승인하면 종료한다.
- 추가 피드백이 있으면 Step 7-1로 복귀하여 루프를 반복한다.
...
- Step별 전체 작업 요약
  ...
  - Step 4. 정적 검증 결과 (누락/불일치 여부, 요구사항 커버리지)
