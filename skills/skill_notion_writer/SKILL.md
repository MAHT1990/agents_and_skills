...
- $$depth 적용 (템플릿에 깊이 절이 있는 경우)
  - 제목 아래 메타 블록(레벨·작성일·조사 범위·다음 확장) 작성 — 형식은 템플릿을 따름 (plain text 코드블록 + 불렛)
  - 섹션별 개수는 템플릿 깊이 표의 상한 이내 — 상한은 채우는 값이 아님
  - 해당 깊이에서 쓰지 않는 섹션은 삭제하지 않고 제목 + 후보 줄(`((L2 에서 작성. 후보: …))`)만 작성, 자리는 템플릿 규칙을 따름
  - 상한 때문에 뺀 내용은 후보 줄·메타 블록의 "다음 확장"에 주제 이름으로만 기록
...
- [ ] 메타 블록이 있고, 생략 섹션이 제목 + 후보 줄로 표기되었는가?
- [ ] 템플릿이 그림을 요구하는 섹션(Feedback Loop 등)에 다이어그램이 있는가?
