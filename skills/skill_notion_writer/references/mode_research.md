...
- 조사 계획 수립
  ...
  - $$template_page 섹션 구조를 미리 파악하여 섹션별로 필요한 자료 항목 도출
    - $$depth 에서 쓰는 섹션에 한해 도출 (생략 섹션은 후보 주제 이름만 확보, 본문 조사 안 함)
    - 섹션별 항목 수는 템플릿 깊이 표의 상한 이내로 계획
- 자료 조사 (도구)
...
- 자료 선별·검증
  ...
  - 채택 자료의 출처(URL·문서명)를 항목별로 보존 → 페이지 References 섹션에 명시
...
- 생성된 페이지에는 반드시 조사 출처(URL·문서명)를 References 섹션에 항목별 명시
  - 직접 확인한 출처만 URL 기재, 개수는 템플릿 깊이 표의 상한 이내
- 출처들을 엮어 작성자가 끌어낸 내용은 제목 끝에 `(해석)` 표기, 근거 출처는 References 의 "해석의 근거" 줄에 기록
...
- [ ] 조사 출처(URL·문서명)가 References 섹션에 항목별 명시되었는가?
- [ ] 추측·창작 없이 확인된 사실만으로 작성되었는가?
- [ ] `(해석)` 과 출처 기반 서술이 구분되었는가? 해석의 근거가 References 에 있는가?
