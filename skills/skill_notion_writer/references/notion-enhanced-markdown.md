...
> 문법 조합 예시일 뿐이다. 섹션 구성·순서·분량의 정본은 Notion 템플릿($$template_page)이며, 아래 섹션 이름은 템플릿과 다를 수 있다.
...
### 실측으로 확인한 함정 (작성 후 fetch 점검 대상)

7. **이스케이프 집합 밖 문자에는 백슬래시 금지**. `_ ( ) & .` 에 `\` 를 붙이면 백슬래시가 그대로 노출된다.
8. **write 도구 content 는 실제 줄바꿈·실제 탭**. 리터럴 `\n`·`\t` 문자열을 넣으면 페이지 전체가 한 블록으로 붕괴한다.
9. **제목 줄은 전체 줄로 치환**. update_content 로 제목 줄의 일부만 바꾸면 `{color="…"}`·`{toggle="true"}` 속성이 떨어지고 토글 자식이 밖으로 나온다. new_str 에 속성과 자식 들여쓰기까지 다시 적는다.
10. **줄머리 `+ `·`N.` 은 목록 기호로 해석**된다 (표 칸·문단·bullet 안 모두). `+` 는 이스케이프도 소용없으므로 문구를 바꾸고, bullet 안 번호는 `1\.` 로 적는다.
11. **bare 파일명·도메인은 자동 링크**된다 (`main.tf` → 가짜 URL). 백틱 인라인 코드로 감싼다.
12. **create-pages 는 본문 최상단 H1 이 title 과 같으면 제거**할 수 있다. 사라지면 update-page 로 H1 을 따로 넣는다.
13. **대용량 한글 본문은 `\uXXXX` 이스케이프 대신 리터럴 한글**로 넘긴다. 이스케이프가 많으면 입력 파싱 실패·유사 음절 오타가 생긴다. 생성 직후 fetch 로 오타를 검수한다.
14. fetch 결과의 `\>` 등은 직렬화 표기일 뿐 UI 에서는 정상인 경우가 많다. 함정과 구분한다.
