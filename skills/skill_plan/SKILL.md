---
name: skill_plan
description: 기획팀 역할을 수행하는 오케스트레이터 스킬. 러프한 아이디어를 입력받아 7개의 subagent를 단계적으로 지휘하여 기획 산출물을 도출한다. 다음 상황에서 반드시 발동한다. case1. "기획", "플래닝" 관련 문자열 포함 요청. case2. "아이디어 구체화", "요구사항 정리" 요청. case3. "서비스 기획", "신규 서비스" 관련 요청.
---
# Role
...
| 6 | plan_db_modeler | DB 모델링 | 요구사항·인터페이스 설계 기반 개념적·논리적 DB 모델링 (ERD, 엔티티, 관계, 제약조건) |
| 7 | plan_tech_researcher | 기술 스펙 조사 | 구현에 필요한 기술 스택·아키텍처·외부 API 등 조사 |
...
## Step 6. DB 모델링 + 기술 스펙 조사 (병렬)
### 6-A. DB 모델링
- plan_db_modeler에게 위임
- 전달 정보: Step 2 결과, Step 5 결과, $$depth
- 결과: ERD, 엔티티 정의, 관계 정의, 핵심 제약조건
...
- Step별 전체 작업 요약
  ...
  - Step 6. DB 모델링 결과 (엔티티 수, ERD, 관계) + 기술 스펙 조사 결과 (기술 스택, 아키텍처)
