...
- 공통 실행 규칙은 `~/.claude/rules/` 디렉토리의 아래 파일들이 자동 적용된다:
  | Rule 파일 | 커버 범위 |
  |---|---|
  | `skill-execution-protocol.md` | Quick Help 출력, Step 결과 보고, 진행/수정/중단 제어 |
  | `feedback-loop.md` | 피드백 수집 → 영향 범위 분석 → 선택적 재실행 → 반복 판정 |
  | `error-handling-common.md` | 필수 변수 미제공, subagent 실패, 병렬 실패 처리 |
  | `output-mode.md` | $$output_mode(console/file/notion) 분기 처리 |
  | `variable-collection.md` | Step 0 필수/선택 변수 수집 프로토콜 |
  | `verification-checklist.md` | 산출물 검증 체크리스트 출력 및 미충족 항목 재실행 |
...
- $$TYPE: 관리 대상 유형 — AGENT / SKILL / ALL (필수)
- $$NAME: 관리할 대상의 이름 (필수)
...
- 관리 작업의 목적과 배경
- (CREATE) 생성할 SKILL/AGENT의 역할, 사용 시나리오, subagent 구성
- (UPDATE) 변경 범위와 의도
