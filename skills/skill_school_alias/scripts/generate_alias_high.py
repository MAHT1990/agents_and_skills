    ...
    if matched_prefix is not None and not matched_prefix.endswith("대학교"):
        ...
        if region is not None:
            ...
            for ab in matched_abbrev_suffixes:
                add(stripped + ab)
            # 범용 고/고교 축약 (비-고등학교 패턴일 때)
            if matched_suffix != "고등학교":
                stripped_general = school_name[:-len("고등학교")][len(region):]
                add(stripped_general + "고")
                add(stripped_general + "고교")
